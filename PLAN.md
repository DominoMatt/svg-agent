...
## Immediate Priority: Phase 0 — Embed-an-LLM Hello World

Before touching any SVG machinery, ship the smallest possible proof-of-concept: a script that **loads a real LLM into our own process**, feeds it a prompt, streams a reply, and exits. Everything else builds on this foundation.

Goal: prove the hard riskiest piece early — that we can reliably load and drive a small quantized model (<~1 GB RAM footprint) from Python on whatever machine we happen to sit on.

See **[Milestone M0](#milestone-m0-hello-world-week-01)** below for acceptance criteria.
...
| `llama-cpp-python` | **Core for Phase 0** embedded LLM inference | GGUF models, CPU/GPU/Metal, streaming |

**Deps breakdown:** 4 lightweight libs + `llama-cpp-python` (compiled ext; wheels for Linux/macOS/Windows, CPU+CUDA). For the **hello-world gate, `llama-cpp-python` is mandatory** — it IS the embedding mechanism. Later, teams preferring zero-native-deps switch to the **Ollama backend**, which needs only `httpx` (already required).

### Model Files (Only for Embedded Backend)

Primary (used by Phase 0 hello world):
...
Both go in `models/` (gitignored). During Phase 0 the model is loaded eagerly at startup (we WANT to feel the cost); in production it degrades to lazy-load on first subjective instruction. **Neither is needed if using the Ollama backend.**
...
Chronological order: **M0 ships first** (see banner atop this page), then M1…M5 expand outward.

### Milestone M0: Hello World (Day 1)
Smallest vertical slice proving we can embed and drive a real LLM in-process.
...
- [x] CLI: `svg-agent propose`, `svg-agent commit`, `svg-agent rollback`
...
Gate 0 — Hello World (blocks everything):
0. **One clone → one greeting** — `make download-model && python examples/hello_world.py` emits a coherent reply from an in-process model, then exits cleanly.

Then the product goals resume:
...
6. Then begin `client.py` — start with `GET /api/conventions` round-trip
7. Implement `markup.py` — test against `fish/current.svg` fixture
8. Wire together in `workflow.py` — first end-to-end edit
